...
const niveles = [
    {
        ...
        descripcion:
            "A post claims that all classes will be canceled tomorrow because of a storm. Investigate the content before it continues to spread.",
        dificultad: "Beginner",
        duracion: "5 minutes",
        recompensa: 100,
        habilidades: ["Sources", "Context", "Evidence"],
        mensaje:
            "The fog is thicker in Viral Plaza. Let us begin our investigation there.",
    ...
    },
    {
        ...
        descripcion:
            "Analyze an answer created by artificial intelligence and uncover facts, quotations, and references that may have been fabricated.",
        dificultad: "Intermediate",
        duracion: "7 minutes",
        recompensa: 100,
        habilidades: ["Responsible AI", "Sources", "Bias"],
        mensaje:
            "Even a clear and convincing answer may contain incorrect information.",
    ...
    },
    {
        ...
        descripcion:
            "Examine photographs and videos to determine whether they were manipulated, generated with AI, or presented out of context.",
        dificultad: "Intermediate",
        duracion: "8 minutes",
        recompensa: 100,
    ...
    },
    {
        ...
        descripcion:
            "Investigate an offer that seems too good to be true and protect the personal data of Infopolis residents.",
        dificultad: "Intermediate",
        duracion: "6 minutes",
        recompensa: 120,
        habilidades: ["Privacy", "Scams", "Security"],
        mensaje:
            "Inspect the offer carefully. Urgency, fees, and credential requests are warning signs.",
    ...
    },
    {
        ...
        descripcion:
            "Decide how to respond to harmful content without escalating conflict or attacking other users.",
        dificultad: "Advanced",
        duracion: "8 minutes",
        recompensa: 150,
        habilidades: ["Empathy", "Dialogue", "Citizenship"],
        mensaje:
            "Moderate the conversation by combining evidence, empathy, and user safety.",
    ...
    },
    {
        ...
        descripcion:
            "A final mission that combines all the investigation skills learned throughout Infopolis.",
        dificultad: "Advanced",
        duracion: "12 minutes",
        recompensa: 250,
        habilidades: ["Investigation", "Comparison", "Decision"],
        mensaje:
            "Build a final case using direct, current, and independently confirmed evidence.",
...
function inicializarProgreso() {
    if (localStorage.getItem("milbotProgressVersion") === VERSION_PROGRESO)
        return;
...
function estaCompletado(nivel) {
    ...
    return niveles.slice(0, indice + 1).every(tieneRegistroCompletado);
}
...
function seleccionarZona(nivel) {
    ...
    zonas.forEach((zona) =>
        zona.classList.toggle("seleccionada", zona.dataset.zona === nivel.id),
    );
    ...
    elementos.panelDisponibilidad.textContent =
        estado === "COMPLETED" ? "Completed" : estado.toLowerCase();
    elementos.mensajeMilbot.textContent = nivel.mensaje;
    elementos.panelHabilidades.replaceChildren(
        ...nivel.habilidades.map((habilidad) => {
...
function mostrarAviso(texto) {
    ...
    temporizadorAviso = setTimeout(
        () => elementos.aviso.classList.remove("visible"),
        3200,
    );
}
...
zonas.forEach((zona, indice) => {
    zona.addEventListener("click", () => {
        ...
        if (estado === "COMING SOON")
            mostrarAviso(`${nivel.titulo} is coming soon`);
    });
});

elementos.botonMision.addEventListener("click", () => {
    if (
        nivelSeleccionado.archivo &&
        estaDesbloqueado(niveles.indexOf(nivelSeleccionado))
    ) {
...
elementos.botonReiniciar.addEventListener("click", () => {
    const confirmar = window.confirm(
        "Restart the entire game? All mission progress and the total score will return to 0.",
    );
    if (!confirmar) return;
...
const siguienteNivel =
    niveles.find(
        (nivel, indice) => estaDesbloqueado(indice) && !estaCompletado(nivel),
    ) || niveles[0];
